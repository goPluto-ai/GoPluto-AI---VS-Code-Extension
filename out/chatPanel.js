...
exports.PlutoAIChatPanel = void 0;
const vscode = __importStar(require("vscode"));
class PlutoAIChatPanel {
    static sendMessageToChat(message) {
    ...
    static createOrShow(extensionUri, provider, context) {
        const column = vscode.window.activeTextEditor?.viewColumn ?? vscode.ViewColumn.Beside;
        if (PlutoAIChatPanel.currentPanel) {
            PlutoAIChatPanel.currentPanel._panel.reveal(column);
            return PlutoAIChatPanel.currentPanel;
        }
        const panel = vscode.window.createWebviewPanel("plutoAiChat", "GoPluto AI Chat", column, {
            enableScripts: true,
            localResourceRoots: [extensionUri],
            retainContextWhenHidden: true,
        });
        PlutoAIChatPanel.currentPanel = new PlutoAIChatPanel(panel, extensionUri, provider, context);
        return PlutoAIChatPanel.currentPanel;
    }
    constructor(panel, extensionUri, provider, context) {
        this._disposables = [];
        this._panel = panel;
    ...
    async _handleSendMessage(message) {
        ...
        try {
            ...
            if (messages.length === 2) {
                vscode.window.showInformationMessage("GoPluto AI: Expert help provided!");
            }
        }
        catch (error) {
...
exports.PlutoAIChatPanel = PlutoAIChatPanel;
function getNonce() {
    let text = "";
